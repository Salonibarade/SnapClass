...
#           st.error("No enrolled students havevoice profiles registerd")
#           return
...
#     df = st.session_state.get('voice_attendance_results')
...
def Voice_attendance_dialog(selected_subject_id):
    ...
    if st.button("Analyze Audio", width="stretch", type="primary"):
        ...
        with st.spinner("Processing Audio data"):
            ...
            st.session_state.voice_attendance_results = (df_results, attendance_to_log)

    # ✅ SAFE READ FROM SESSION STATE
    data = st.session_state.get("voice_attendance_results")

    if data:
        df_results, logs = data
